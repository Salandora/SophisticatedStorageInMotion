...
public class EmiCompat implements EmiPlugin {
	...
	public void register(EmiRegistry registry) {
		...
		registry.setDefaultComparison(ModItems.STORAGE_MINECART, movingStorageComparison);

		registry.addRecipeHandler(ModEntities.MOVING_STORAGE_CONTAINER_TYPE, EmiGridMenuInfo.crafting());
		registry.addRecipeHandler(ModEntities.MOVING_STORAGE_CONTAINER_TYPE, EmiGridMenuInfo.stonecutting());
		registry.addRecipeHandler(ModEntities.MOVING_STORAGE_CONTAINER_TYPE, EmiGridMenuInfo.smithing());
	}

	private static void registerRecipes(EmiRegistry registry, Iterable<CraftingRecipe> recipes) {
		recipes.forEach(recipe -> {
			...
			registry.addRecipe(new EmiCraftingRecipe(
					...
					new ResourceLocation(recipe.getId().getNamespace(), "/" + recipe.getId().getPath())
				)
			);
		});
	}
