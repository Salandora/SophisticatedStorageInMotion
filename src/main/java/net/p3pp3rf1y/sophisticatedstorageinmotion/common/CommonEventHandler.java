...
public class CommonEventHandler {
	...
	private static void onMovingStorageUncrafted(Player player, ItemStack result, Container craftMatrix) {
		...
		@Nullable UUID storageId = result.sophisticatedFabricLibrary_get(ModCoreDataComponents.STORAGE_UUID);
		...
		contents.put(StorageWrapper.RENDER_INFO_TAG, result.sophisticatedFabricLibrary_getOrDefault(ModCoreDataComponents.RENDER_INFO_TAG, CustomData.EMPTY).copyTag());
	...
	private static void onMovingStorageCraftedFromShulkerBox(Player player, ItemStack result, Container craftMatrix) {
		...
		if (storageItem.getItem() instanceof ShulkerBoxItem) {
				...
				shulkerStorageWrapper.getContentsUuid().ifPresent(id -> {
					...
					storageItem.sophisticatedFabricLibrary_set(ModCoreDataComponents.RENDER_INFO_TAG, CustomData.of(contentsNbt.getCompound(StorageWrapper.RENDER_INFO_TAG)));
